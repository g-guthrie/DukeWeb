...
static void on_vmode3dcombo_changed(GtkComboBox *combobox, gpointer user_data)
{
    ...
    settings.xdim = validmode[val].xdim;
    settings.ydim = validmode[val].ydim;
    settings.bpp = validmode[val].bpp;
}
...
static void on_inputdevcombo_changed(GtkComboBox *combobox, gpointer user_data)
{
    ...
    {
    case 0: settings.usemouse = 0; settings.usejoystick = 0; break;
    case 1:	settings.usemouse = 1; settings.usejoystick = 0; break;
    case 2:	settings.usemouse = 0; settings.usejoystick = 1; break;
    case 3:	settings.usemouse = 1; settings.usejoystick = 1; break;
    }
}
...
{
    UNREFERENCED_PARAMETER(user_data);
    settings.noautoload = !gtk_toggle_button_get_active(togglebutton);
}
...
static void PopulateForm(unsigned char pgs)
{
    if ((pgs == ALL) || (pgs == POPULATE_VIDEO))
    {
        ...
        mode3d = videoCheckMode(&settings.xdim, &settings.ydim, settings.bpp, settings.fullscreen, 1);
        if (mode3d < 0)
        {
            ...
            for (; cd[i]; i++)
            {
                mode3d = videoCheckMode(&settings.xdim, &settings.ydim, cd[i], settings.fullscreen, 1);
    ...
    if ((pgs == ALL) || (pgs == POPULATE_CONFIG))
    {
        ...
        {
        case 0: if (settings.usejoystick)
            ...
            break;
        case 1:	if (settings.usejoystick)
        ...
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(stwidgets.autoloadcheck), !settings.noautoload);
...
int32_t startwin_run(void)
{
    ...
    settings.usejoystick = ud.config.UseJoystick;
//    settings.game = g_gameType;
    settings.noautoload = g_noAutoLoad;
    ...
    if (retval) // launch the game with these parameters
    {
        ...
        ud.config.UseJoystick = settings.usejoystick;
//        g_gameType = settings.game;
        g_noAutoLoad = settings.noautoload;
