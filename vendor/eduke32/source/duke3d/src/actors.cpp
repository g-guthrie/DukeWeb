...
void A_MoveCyclers(void)
{
    for (bssize_t i=g_cyclerCnt-1; i>=0; i--)
    {
        ...
        if ((unsigned)sectNum >= (unsigned)numsectors)
        {
            ...
            static int loggedInvalidCyclers = 0;
            if (loggedInvalidCyclers < 8)
            {
                LOG_F(ERROR, "web invalid cycler[%d] sect=%d numsectors=%d", (int)i, sectNum, numsectors);
                loggedInvalidCyclers++;
            }
#endif
            continue;
        }
        ...
        {
            if ((unsigned)sector[sectNum].wallptr >= (unsigned)numwalls
                || (unsigned)(sector[sectNum].wallptr + sector[sectNum].wallnum) > (unsigned)numwalls)
            {
#ifdef __EMSCRIPTEN__
                static int loggedInvalidCyclerWalls = 0;
                if (loggedInvalidCyclerWalls < 8)
                {
                    LOG_F(ERROR, "web invalid cycler[%d] wall range ptr=%d count=%d numwalls=%d",
                          (int)i, sector[sectNum].wallptr, sector[sectNum].wallnum, numwalls);
                    loggedInvalidCyclerWalls++;
                }
#endif
                continue;
            }
            ...
            for (bssize_t wallsLeft = sector[sectNum].wallnum; wallsLeft > 0; wallsLeft--, pWall++)
            {
                if (pWall->hitag != 1)
                {
                    ...
                    if ((pWall->cstat&2) && (unsigned)pWall->nextwall < (unsigned)numwalls)
