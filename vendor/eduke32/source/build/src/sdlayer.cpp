...
#ifdef __EMSCRIPTEN__
static inline void webPublishSdlKeyEvent(char const *kind, SDL_KeyboardEvent const &key, int32_t translated)
{
    EM_ASM({
        const host = globalThis.__edukeHost || (globalThis.__edukeHost = {});
        const events = host.sdlEvents || (host.sdlEvents = globalThis.__edukeSdlEvents || []);
        let event = {};
        event.kind = UTF8ToString($0);
        event.scancode = $1;
        event.sym = $2;
        event.translated = $3;
        event.mod = $4;
        ...
        if (events.length > 96)
            events.shift();
        globalThis.__edukeSdlEvents = events;
    }, kind, (int)key.keysym.scancode, (int)key.keysym.sym, translated, (int)key.keysym.mod, (int)key.repeat);
}

static inline void webPublishSdlMouseMotion(SDL_MouseMotionEvent const &motion)
{
    EM_ASM({
        const host = globalThis.__edukeHost || (globalThis.__edukeHost = {});
        let mouse = host.sdlMouse || (host.sdlMouse = globalThis.__edukeSdlMouse || {});
        mouse.kind = "motion";
        mouse.x = $0;
        mouse.y = $1;
        mouse.xrel = $2;
        mouse.yrel = $3;
        mouse.state = $4;
        globalThis.__edukeSdlMouse = mouse;
    }, motion.x, motion.y, motion.xrel, motion.yrel, motion.state);
}

static inline void webPublishSdlMouseButton(char const *kind, SDL_MouseButtonEvent const &button)
{
    EM_ASM({
        const host = globalThis.__edukeHost || (globalThis.__edukeHost = {});
        let mouse = host.sdlMouse || (host.sdlMouse = globalThis.__edukeSdlMouse || {});
        mouse.kind = UTF8ToString($0);
        mouse.button = $1;
        mouse.state = $2;
        mouse.x = $3;
        mouse.y = $4;
        globalThis.__edukeSdlMouse = mouse;
    }, kind, (int)button.button, (int)button.state, button.x, button.y);
}
#endif
...
#endif
#if defined __EMSCRIPTEN__ && defined SDL_HINT_EMSCRIPTEN_KEYBOARD_ELEMENT
    // Keep SDL keyboard events bound to the game canvas so browser focus is explicit.
...
void sdlayer_setvideomode_opengl(void)
{
    ...
    LOG_F(INFO, "web gl stage: opengl setup enter");
    glsurface_destroy();
    LOG_F(INFO, "web gl stage: after glsurface_destroy");

#ifndef __EMSCRIPTEN__
    glShadeModel(GL_SMOOTH);  // GL_FLAT
#endif
    LOG_F(INFO, "web gl stage: after glShadeModel");
    glClearColor(0, 0, 0, 1.0);  // Black Background
    LOG_F(INFO, "web gl stage: after glClearColor");
    glHint(GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST);  // Use FASTEST for ortho!
    LOG_F(INFO, "web gl stage: after glHint");
//    glHint(GL_LINE_SMOOTH_HINT, GL_NICEST);
    glViewport(0,0,xres,yres);
    LOG_F(INFO, "web gl stage: after glViewport");

#ifndef EDUKE32_GLES
    glDisable(GL_DITHER);
    LOG_F(INFO, "web gl stage: after glDisable(GL_DITHER)");
#endif

    fill_glinfo();
    LOG_F(INFO, "web gl stage: after fill_glinfo");
}
...
int setvideomode_sdlcommonpost(int32_t x, int32_t y, int32_t c, int32_t fs, int32_t regrab)
{
    ...
    if (!nogl)
    {
        LOG_F(INFO, "web gl stage: before sdlayer_setvideomode_opengl");
        sdlayer_setvideomode_opengl();
        LOG_F(INFO, "web gl stage: after sdlayer_setvideomode_opengl");
    }
#endif
...
#endif
    {
        ...
        if (!sdl_surface)
        {
            LOG_F(ERROR, "Unable to refresh window surface after video mode change: %s.", SDL_GetError());
            return -1;
        }
    }
#endif
...
int32_t videoSetMode(int32_t x, int32_t y, int32_t c, int32_t fs)
{
    ...
    LOG_F(INFO, "web gl stage: before setvideomode_sdlcommonpost");
    setvideomode_sdlcommonpost(x, y, c, fs, regrab);
    LOG_F(INFO, "web gl stage: after setvideomode_sdlcommonpost");

#if MICROPROFILE_ENABLED != 0 && !defined(__EMSCRIPTEN__)
    ...
    LOG_F(INFO, "web gl stage: leaving videoSetMode");
    return 0;
}
#endif
...
#ifdef __EMSCRIPTEN__
    static int markedShowFrame = 0;
    if (!markedShowFrame)
    {
        wm_setapptitle("WEB SHOWFRAME");
        LOG_F(INFO, "web videoShowFrame entry");
        markedShowFrame = 1;
    }
#endif
...
#ifdef __EMSCRIPTEN__
    static int markedShowFramePalette = 0;
    if (!markedShowFramePalette)
    {
        wm_setapptitle("WEB SHOWFRAME PAL");
        LOG_F(INFO, "web videoShowFrame after palette");
        markedShowFramePalette = 1;
    }
#endif

#ifdef __EMSCRIPTEN__
    static int markedBlitSource = 0;
    if (markedBlitSource < 6)
    {
        int srcNonZero = 0;
        int palNonZero = 0;
        auto const *src = softsurface_getBuffer();
        vec2_t const srcRes = softsurface_getBufferResolution();
        int const sampleCount = min(srcRes.x * srcRes.y, 4096);
        for (int i = 0; i < sampleCount; ++i)
            srcNonZero += src[i] != 0;
        for (int i = 0; i < 256; ++i)
            palNonZero += curpalettefaded[i].r != 0 || curpalettefaded[i].g != 0 || curpalettefaded[i].b != 0;
        LOG_F(INFO, "web blit #%d source nonzero=%d palette nonzero=%d sampleCount=%d", markedBlitSource + 1, srcNonZero, palNonZero, sampleCount);
        if (markedBlitSource == 0 || srcNonZero > 0)
            wm_setapptitle(srcNonZero ? "WEB BLIT SRCNZ" : "WEB BLIT SRC0");
        markedBlitSource++;
    }
#endif
...
#ifdef __EMSCRIPTEN__
    static int markedAfterBlit = 0;
    if (!markedAfterBlit)
    {
        LOG_F(INFO, "web softsurface_blitBuffer returned");
        markedAfterBlit = 1;
    }
#endif
...
#ifdef __EMSCRIPTEN__
    static int markedWindowSurface = 0;
    if (!markedWindowSurface)
    {
        LOG_F(INFO, "web SDL_UpdateWindowSurface done");
        markedWindowSurface = 1;
    }
#endif
#else
...
#ifdef __EMSCRIPTEN__
            webPublishSdlMouseMotion(ev->motion);
#endif
#ifndef GEKKO
...
# endif
        ...
        {
#ifdef __EMSCRIPTEN__
            webPublishSdlMouseButton(ev->type == SDL_MOUSEBUTTONDOWN ? "buttondown" : "buttonup", ev->button);
#endif
            int32_t j;
...
#ifdef __EMSCRIPTEN__
                webPublishSdlKeyEvent(ev.type == SDL_KEYDOWN ? "keydown" : "keyup", ev.key, code);
#endif
