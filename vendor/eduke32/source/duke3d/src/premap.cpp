...
{    
    ...
    {
        static int invalidTileWarnings = 0;
        if (invalidTileWarnings < 16)
            LOG_F(WARNING, "Ignoring invalid precache tile %d (type %d)", tile, type);
        invalidTileWarnings++;
        return;
    }
...
static void tloadtile(int tilenume, int type)
{
    ...
    if ((unsigned)tilenume >= MAXTILES)
    {
        flag_precache(tilenume, type);
        return;
    }
    ...
    firstTile = clamp(firstTile, 0, MAXTILES - 1);
    lastTile  = clamp(lastTile, 0, MAXTILES - 1);
