...
int engineFPSLimit(bool const throttle)
{
    ...
    // In the browser, the outer main loop is already paced by the platform.
    // Only apply an internal limiter when the user explicitly asks for a
    // positive cap. Do not infer timing from refreshfreq, which is not a
    // reliable browser concept.
    (void) throttle;
    ...
    g_frameDelay = calcFrameDelay(r_maxfps);
#else
    double const effectiveRefreshFreq = refreshfreq > 1.0 ? refreshfreq : 60.0;
    g_frameDelay = calcFrameDelay(!throttle || ((unsigned)(r_maxfps-1) < (unsigned)effectiveRefreshFreq) ? r_maxfps : -1);
#endif
    ...
    {
#ifdef __EMSCRIPTEN__
        static int loggedZeroFrameDelay = 0;
        if (loggedZeroFrameDelay < 8)
        {
            LOG_F(WARNING, "web engineFPSLimit saw zero frame delay (throttle=%d r_maxfps=%d refreshfreq=%.3f)",
                  (int)throttle, r_maxfps, refreshfreq);
            loggedZeroFrameDelay++;
        }
#endif
        nextFrameTicks = frameTicks + 1;
        return true;
    }

    if (frameTicks >= nextFrameTicks)
    {
        // Avoid a potentially unbounded catch-up loop when browser timing jumps
        // or the scheduler stalls for an extended interval.
        uint64_t const framesBehind = ((frameTicks - nextFrameTicks) / g_frameDelay) + 1;
        nextFrameTicks += framesBehind * g_frameDelay;

        if (nextFrameTicks <= frameTicks)
            nextFrameTicks = frameTicks + g_frameDelay;
