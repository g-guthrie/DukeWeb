...
bool tileLoad(int16_t tileNum)
{
    ...
    browserPublishTileLoad(tileNum, dasiz, 1);

    // Allocate storage if necessary.
    if (waloff[tileNum] == 0)
    {
        walock[tileNum] = CACHE1D_UNLOCKED;
        browserPublishTileLoad(tileNum, dasiz, 2);
        g_cache.allocateBlock(&waloff[tileNum], dasiz, &walock[tileNum]);
    }

    browserPublishTileLoad(tileNum, dasiz, 3);
    ...
    browserPublishTileLoad(tileNum, dasiz, 4);
    tilePostLoad(tileNum);

    browserPublishTileLoad(tileNum, dasiz, 5);
