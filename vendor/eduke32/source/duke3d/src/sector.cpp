...
void G_AnimateWalls(void)
{
    for (bssize_t animwallNum = g_animWallCnt-1; animwallNum>=0; animwallNum--)
    {
        ...
        if ((unsigned)wallNum >= (unsigned)numwalls)
        {
            ...
            static int loggedInvalidAnimWalls = 0;
            if (loggedInvalidAnimWalls < 8)
            {
                LOG_F(ERROR, "web invalid animwall[%d] wallnum=%d numwalls=%d", (int)animwallNum, wallNum, numwalls);
                loggedInvalidAnimWalls++;
            }
#endif
            continue;
        }
